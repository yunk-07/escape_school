...
class ItemSpawner {
  ...
  static double _applyRarityBoost(
    double baseWeight,
    int level,
    double rarityBoost,
  ) {
  ...
  static Item? _selectRandomItem({double rarityBoost = 0.0}) {
    ...
    for (final item in spawnableItems) {
      final w = _applyRarityBoost(
        _getSpawnProbability(item.level),
        item.level,
        rarityBoost,
      );
      weights[item] = w;
  ...
  static bool _isValidSpawnPosition(
    ...
    for (final chestPos in chestPositions) {
      final distanceToChest = sqrt(
        pow(position.x - chestPos.x, 2) + pow(position.y - chestPos.y, 2),
      );
  ...
  static MapEntry<Point<int>, Item>? trySpawnItem(
    ...
    // 随机选择一个有效位置
    final spawnPosition =
        validPositions[_random.nextInt(validPositions.length)];
  ...
  static int getNextSpawnInterval() {
    return _baseSpawnInterval +
        _random.nextInt(_spawnVariation * 2 + 1) -
        _spawnVariation;
  }
  ...
  /// 为宝箱生成多个随机物品
  static List<Item> generateChestItems({
    int minItems = 1,
    int maxItems = 3,
    double rarityBoost = 0.0,
  }) {
  ...
  static Item? selectRandomChestItemAt(
    Point<int> chestPosition, {
    double rarityBoost = 0.0,
  }) {
    final chestItems = allItems.where((item) => item.level >= 0).toList();
    if (chestItems.isEmpty) return null;
    ...
    for (final item in chestItems) {
      ...
      final multiplier =
          inBuilding ? (buildingMultipliers[item.level] ?? 1.0) : 1.0;
  ...
  static List<Item> generateChestItemsAtPosition(
    Point<int> chestPosition, {
    int minItems = 1,
    int maxItems = 3,
    double rarityBoost = 0.0,
    ...
    for (int i = 0; i < itemCount; i++) {
      final item = selectRandomChestItemAt(
        chestPosition,
